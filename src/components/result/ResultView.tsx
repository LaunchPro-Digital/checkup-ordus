  ...
  return (
    <div className="space-y-10 motion-safe:animate-fade-in">
      ...
      <nav
        ...
        className="flex flex-wrap gap-2 sticky top-[76px] z-20 py-2 -mx-4 px-4 backdrop-blur-md"
        style={{
      ...
      {/* ── Header — h1 inherits Fractul Black from global CSS, no font-display needed ── */}
      ...
      {/* ── CRP Gauge — DS: card-featured (glow roxo) ── */}
      <div className="card-featured p-8 md:p-12" id="gauge">
      ...
      {/* ── Fallback actions — shown only when AI finished without content ── */}
      {!isAiLoading && !aiContent && (
        // DS: card-featured = bg accent/5 + border accent/40 + shadow-purple
        <div className="card-featured p-6 md:p-8">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground text-base mt-2">
              Com base no seu diagnóstico, estas são as 3 ações com maior impacto para reduzir
              o risco percebido pelo seu cliente
            </p>
          </div>
                ...
                <div key={index} className="space-y-4">
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-accent/20 flex items-center justify-center">
                      <span className="font-black text-xl text-accent">{index + 1}</span>
                    </div>
                    <div className="flex-1 space-y-3">
                      ...
                      <h3 className="text-lg font-black text-foreground">{action.action}</h3>
                  ...
                  {index < detailedResult.actions.length - 1 && (
                    <div style={{ borderTop: '1px solid rgba(255,255,255,.06)', marginTop: '1.5rem' }} />
                  )}
                </div>
              );
            })}
      ...
      {/* ── Machine explanation — DS: card-elevated ── */}
      ...
      {/* ── Footer CTA — DS: cta-box-glass ── */}
      <div className="cta-box-glass p-8 text-center space-y-6" id="cta">
